	...
	<%@include file="/managers/top.jsp"%>
        <div id="page-wrapper">
			<div class="col-md-12 graphs">
				<div class="xs">
					<h3>管理员列表     <button type="button" class="btn-primary btn" onclick="add()">增加</button></h3>
							...
							<table class="table table-striped">
								...
								<s:iterator  value="admins">
									<tr>
										...
										<td><button type="button" onclick="toupdate('<s:property value="id"/>')" class="label btn_lg label-primary" style="padding:8px 12px">修改</button><c:if test="${id!=admin.id}"><button type="button" onclick="todelete('<s:property value="id"/>')"  class="label btn_lg label-danger" style="padding:8px 12px">删除</button></c:if></td>
									</tr>
								</s:iterator>
								</tbody>
							</table>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body">
									<form  id="add" action="<%=Const.ROOT%>admin_add"class="form-floating ng-pristine ng-invalid ng-invalid-required ng-valid-email ng-valid-url ng-valid-pattern" novalidate="novalidate" ng-submit="submit()">
										<fieldset>
											<div class="form-group">
												<label class="control-label">账号</label>
												<input type="text" class="form-control1 ng-invalid ng-invalid-required ng-touched" name="admin.username"  required>
											</div>
											<div class="form-group">
												<label class="control-label">密码</label>
												<input type="password" class="form-control1 ng-invalid ng-invalid-required ng-touched" name="admin.password"  required>
											</div>
										</fieldset>
									</form>
								</div>
	...
	<div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body">
									<form id="update" method="post" enctype="multipart/form-data" action="<%=Const.ROOT%>admin_doupdate" class="form-floating ng-pristine ng-invalid ng-invalid-required ng-valid-email ng-valid-url ng-valid-pattern" novalidate="novalidate" ng-submit="submit()">
										<input type="hidden" name="admin.id" id="id"/>
										<input type="hidden" name="admin.username" id="username"/>
										<fieldset>
											<div class="form-group">
												<label class="control-label">密码</label>
												<input type="text" id="password" class="form-control1 ng-invalid ng-invalid-required ng-touched" name="admin.password" required>
											</div>
										</fieldset>
									</form>
								</div>
...
<script>
    function goPage(page){
        location="admin_list?pageBean.pageNo="+page;
    }
    ...
    function doadd(){
     ...
     function toupdate(id){
        $.getJSON("<%=Const.ROOT%>adminjson_toupdate",{id:id},function(data){
    ...
    function dodelete(){
        ...
        location="<%=Const.ROOT%>admin_delete?id="+id;
    }
</script>
</body>
</html>
